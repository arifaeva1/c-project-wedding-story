...
namespace wedding_test_2
{
    public partial class bookstage : Form
    {
        ...
        public bookstage(int clientID, int orderID)
        {
            InitializeComponent();
            BindGridView();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                {
                    SqlConnection con = new SqlConnection(cs);
                    String sql = "INSERT INTO [OrderTable] (ClientID, StageID, Status)" +
                                " VALUES(@ClientID, @StageID, 'unpaid');" +
                                " SELECT SCOPE_IDENTITY()";
                    SqlCommand cmd = new SqlCommand(sql, con);
                    cmd.Parameters.AddWithValue("@ClientID", clientID);
                    cmd.Parameters.AddWithValue("@StageID", textBox1.Text);
                ...
                {
                    SqlConnection con = new SqlConnection(cs);
                    String sql = "UPDATE [OrderTable] SET ClientID = @ClientID, StageID = @StageID" +
